...
def parse_arguments():
	...
	parser.add_argument('-c', '--compression', help="compression algorithm", 
		type=str,choices=['lzma', 'gzip', 'bzip2', 'zlib', 'lz4', 'snappy'], required=True)
...
def compress_snappy(sequence):
	return snappy.compress(sequence)
...
def compressed_size(sequences, algorithm, saveCompression, comparison):
	...
	if algorithm == 'snappy':
		compressed_seq1 = compress_snappy(sequences[0])
		compressed_seq2 = compress_snappy(sequences[1])
		compressed_seqconcat = compress_snappy(sequences[2])
