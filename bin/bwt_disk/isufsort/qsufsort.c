...
/* Makes suffix array p of x. x becomes inverse of p. p and x are both of size
   n+1. Contents of x[0...n] are integers in the range 0 ... asize-1 0*/
...
#if 0   
   // the current transform() procedure assumes the file ends
   // with a eof. So we cannot use it
...
#else
   r=1;
   bucketsort(V, I, n, asize);   /* bucketsort on first r positions.*/   
#endif
   
   h=r;                         /* number of symbols aggregated by transform.*/
   
   while (*I>=-n) {
     if (_bwtext_Verbose>3)     // report the current sorting status
       fprintf(stderr,"Suffsort h=%d, groups=%d\n",h,*I+n);
...
/* Sort the suffixes and return the rank array (isa) of x.
   p and x are both of size n+1.
   Contents of x[0...n] are integers in the range 0 ... asize-1
   x[n] is different from any other symbol in x[]
   The rank array is overwritten to x. 
   p is used but its initial and final states are irrelevant */
...
/* Makes suffix array p of x. x becomes inverse of p. p and x are both of size
   n+1. Contents of x[0...n-1] are integers in the range 0 ... asize-1 
   Original contents of x[n] is disregarded, the n-th symbol being regarded as
...
void _bwtext_ls_suffixsort_plain(int *x, int *p, int n, int asize)
{
   ...
   if (n>=asize) {                /* if bucketing possible,*/
      j=transform(V, I, n, asize, 0, n);
      bucketsort(V, I, n, j);   /* bucketsort on first r positions.*/
   } else {
      transform(V, I, n, asize, 0, INT_MAX);
      for (i=0; i<=n; ++i)
         I[i]=i;                /* initialize I with suffix numbers.*/
      h=0;
      sort_split(I, n+1);       /* quicksort on first r positions.*/
