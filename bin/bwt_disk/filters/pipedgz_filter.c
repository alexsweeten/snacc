...
/* **************************************************************
   ...
   ************************************************************** */
void *_bwtext_pipedgz_open(char *fname, int32 mode)
{ 
  piped_file *f;

  if(sizeof(off_t)<8) 
    _bwtext_fatal_error("_bwtext_pipedgz_open: " 
			"your system does't support >4GB files");

  if(mode=='r') {
    f = (piped_file *) malloc(sizeof(piped_file));
    if(f==NULL) _bwtext_out_of_mem("_bwtext_pipedgz_open");
    f->filename = strdup(fname);
    if(f->filename==NULL) _bwtext_out_of_mem("_bwtext_pipedgz_open");
    asprintf(&(f->command),"gzip -cd %s",fname);
    if(f->command==NULL) _bwtext_out_of_mem("_bwtext_pipedgz_open");
    // fprintf(stderr,"***** Opening (R): %s\n",f->command); 
    f->stream = popen(f->command,"r");  
    if (f->stream==NULL)
    _bwtext_fatal_error("_bwtext_pipedgz_open: unable to open pipe");
    f->type = 0;
    return f;
  }
  ...
  if(mode=='w') {
    ...
    f->filename = strdup(fname);
    if(f->filename==NULL) _bwtext_out_of_mem("_bwtext_pipedgz_open");
    asprintf(&(f->command), "gzip -c > %s",fname);
    if(f->command==NULL) _bwtext_out_of_mem("_bwtext_pipedgz_open");
  ...
  _bwtext_fatal_error("_bwtext_pipedgz_open (illegal mode)");
  return NULL;
}


int64 _bwtext_pipedgz_close(void *w)
{
...
int32 _bwtext_pipedgz_read(void *w, uint8 *b, int32 n)
{
  piped_file *f = (piped_file *) w;
...
int32 _bwtext_pipedgz_write(void *w, uint8 *b, int32 n)
{
  piped_file *f = (piped_file *) w;
...
void _bwtext_pipedgz_rewind(void *w)
{
  piped_file *f = (piped_file *) w;
  ...
  if(ris!=0) {
    ...
    // _bwtext_fatal_error("_bwtext_pipedgz_rewind: error closing pipe");
  }

  f->stream = popen(f->command,"r");  
  if (f->stream==NULL)
    _bwtext_fatal_error("_bwtext_pipedgz_rewind: unable to open pipe");
}
...
void _bwtext_register_pipedgz_filter(filter *t)
{
  t->name = "piped gzip";
  t->extension = ".gz";
  t->open = _bwtext_pipedgz_open;
  t->read = _bwtext_pipedgz_read;
  t->write = _bwtext_pipedgz_write;
  t->rewind = _bwtext_pipedgz_rewind;
  t->close = _bwtext_pipedgz_close;
}
